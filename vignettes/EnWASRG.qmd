...
allD$RIAGENDR = as.factor(allD$RIAGENDR)
```

```{r supportFuns}

##chatGPT generated function to summarize the columns in a data.frame suitable 
## putting through kable to get a Table.
...
summarize_df <- function(df) {
  dd = data.frame(
    ...
    Values = sapply(df, function(col) {
       if (is.logical(col)) {
        vals <- c("TRUE", "FALSE")
        counts <- sapply(vals, function(v) sum(col == v, na.rm = TRUE))
        paste(sprintf("%s (%d)", vals, counts), collapse = ", ")
      } else if (is.factor(col)) {
        vals <- levels(col)
        counts <- sapply(vals, function(v) sum(col == v, na.rm = TRUE))
        paste(sprintf("%s (%d)", vals, counts), collapse = ", ")
        ...
        paste(signif(rng[1], 3), signif(rng[2], 3), sep = " – ")
      } else {
        NA
      }
    })
...
In Table XX we summarize the data over all the cycles.

```{r table1}

dt = summarize_df(allD[,c(1,4:9)])
dt |>
  kbl() |>
...
```{r basemodel}
## FIXME - Deepayan, can you look at the anova stuff - I think we want some
## sort of helper function to get the AIC etc - or perhaps just drop that
## also the df for both age and sex includes the 5 terms from spline(age):sex
## I think that is right, as you have to drop both 

##I did test RIAGENDR*race, and ns(RIDAGEYR, df = 5) * RIAGENDR*race and nothing
## seemed significant - also obese*race - which came close

combModel <- 'SYSTOLIC ~ ns(RIDAGEYR, df = 5) * RIAGENDR + race + obese + DMDEDUC2 + lowincome + Cycle'

## this looks for race by obesity interactions - but finds none so is
## omitted for now
#c2 <- 'SYSTOLIC ~ ns(RIDAGEYR, df = 5) * RIAGENDR + race*obese + DMDEDUC2 + lowincome + Cycle'
#lm2 <-  lm(formula=c2, data=allD)

lmC = lm(formula=combModel, data=allD)

kable(summary(lmC)$coefficients)

## or we could just drop the "heading" attribute
ftesttable = data.frame(drop1(lmC, test= "F"))

## check using obese
#noObeseM <- 'SYSTOLIC ~ ns(RIDAGEYR, df = 5) * RIAGENDR + race + DMDEDUC2 + lowincome + Cycle'
#lmNOb = lm(formula= noObeseM, data=allD)
#data.frame(anova(lmNOb, lmC))

noAgeM <- 'SYSTOLIC ~ RIAGENDR + race + obese + DMDEDUC2 + lowincome + Cycle'

lmnoA = lm(formula=noAgeM, data=allD)

agemM = data.frame(anova(lmnoA, lmC))
mAIC = extractAIC(lmnoA)
subv = c(agemM[2, c(3,4)], agemM[1,2], mAIC[2], agemM[2, 5:6])
names(subv) = names(ftesttable)
ftesttable = rbind(ftesttable, "age*" = subv)

noSexM = 'SYSTOLIC ~ ns(RIDAGEYR, df = 5) + race + obese + DMDEDUC2 + lowincome + Cycle'
lmnoS = lm(formula = noSexM, data = allD)

##probably want a function to do this - so it is not so kludgy
sexM = data.frame(anova(lmnoS, lmC))
mAIC = extractAIC(lmnoS)
subv = c(sexM[2, c(3,4)], sexM[1,2], mAIC[2], sexM[2, 5:6])
names(subv) = names(ftesttable)
ftesttable = rbind(ftesttable, "sex*" = subv)
ftesttable$"Pr..F." = round(ftesttable$"Pr..F.", digits=3)

ftesttable |> kbl(caption = "Changes in Fit Dropping Terms") |>
    kable_classic()

```

### Examine Age and Sex interactions (by race)

From the analysis of the effects of dropping different terms we see that there are pretty substantial effects due to age, sex and as indicated by our research there does seem to be a pretty substantial interaction between them affecting systolic blood pressure.  We can examine estimates of the effects using the fitted model.  We will set race to `White`, use cycle `_J`, education at `HS`, obese to be false and low income to be false.  We can see that the interaction is quite strong, with women having lower blood pressure at younger ages but on average overtaking men after sixty and then having higher blood pressure in their later years.  Systolic blood pressure is higher for blacks, than whites.  We did not explore other higher level interactions, which certainly could be present in the data and a more complete attempt at modeling might try to uncover whether there are substantial interctions that should also be accounted for.

```{r predictionofAgeSex}
 ##first predict males
  yvals = seq(31, 79,by=1)
  dfpred1 = data.frame(RIDAGEYR=yvals, RIAGENDR=rep("Male", 49), obese=rep(FALSE, 49), DMDEDUC2=rep("HS", 49), race=rep("White", 49), Cycle = rep("_J", 49), lowincome=rep(FALSE, 49))

    dfpred1B = data.frame(RIDAGEYR=yvals, RIAGENDR=rep("Male", 49), obese=rep(FALSE, 49), DMDEDUC2=rep("HS", 49), race=rep("Black", 49), Cycle = rep("_J", 49), lowincome=rep(FALSE, 49))

  dfpred2 = data.frame(RIDAGEYR=yvals, RIAGENDR=rep("Female", 49), obese=rep(FALSE, 49), DMDEDUC2=rep("HS", 49), race=rep("White", 49), Cycle = rep("_J", 49), lowincome=rep(FALSE, 49))
  dfpred2B = data.frame(RIDAGEYR=yvals, RIAGENDR=rep("Female", 49), obese=rep(FALSE, 49), DMDEDUC2=rep("HS", 49), race=rep("Black", 49), Cycle = rep("_J", 49), lowincome=rep(FALSE, 49))

  predVM = predict(lmC, newdata=dfpred1) ##MW
  predVMB = predict(lmC, newdata = dfpred1B) ##MB
  predVF = predict(lmC, newdata=dfpred2) ##FW
   predVFB = predict(lmC, newdata=dfpred2B) ##FB
  # lines(40:85, predV)
  plot(31:79, predVM, type="l", xlab="Age", ylab = "Systolic BP", 
       main="Age by Sex Interaction", ylim=c(100,150),lwd=2)
  lines(31:79, predVF, col="skyblue", lwd=2)
  lines(31:79, predVMB, col="green", lwd=2)
  lines(31:79, predVFB, col="red", lwd=2)
 legend("topleft", c("WM", "WF", "BM", "BF"), col=c("black", "skyblue","green", "red"),
        lwd=2)
```
